...
(function () {
  ...
  var CHAT_OPTIONS = [
    ["Request a quote", "Hello Mimshak Pak, I would like a quote for packaging. Here is what I need:"],
    ["Ask about pricing", "Hello Mimshak Pak, please what does your packaging cost? I am looking at:"],
    ["Check on my order", "Hello Mimshak Pak, please can I get an update on my order? My invoice number is:"],
    ["Request a sample", "Hello Mimshak Pak, I would like to request a sample before placing a bulk order."],
    ["Something else", "Hello Mimshak Pak,"]
  ];

  function isOpenNow() {
    var d = new Date(), day = d.getDay(), h = d.getHours();
    return day >= 1 && day <= 6 && h >= 8 && h < 18; // Mon-Sat, 8am-6pm
  }

  function buildChatPanel(trigger, waIcon) {
    var open = isOpenNow();
    ...
    panel.innerHTML =
      '<div class="chat-head">' +
        '<div class="chat-avatar">' + waIcon + "</div>" +
        "<div><strong>Mimshak Pak</strong>" +
        '<span class="chat-status ' + (open ? "on" : "off") + '">' +
          (open ? "Usually replies within minutes" : "Closed now — we reply next working day") +
        "</span></div>" +
        '<button class="chat-x" type="button" aria-label="Close chat">&times;</button>' +
      "</div>" +
      '<div class="chat-body">' +
        "<p class=\"chat-greet\">Hello! What can we help you with?</p>" +
        CHAT_OPTIONS.map(function (o, i) {
          return '<button class="chat-opt" type="button" data-i="' + i + '">' + o[0] + "</button>";
        }).join("") +
      "</div>" +
      '<div class="chat-foot">Opens WhatsApp • ' + CONTACT.phoneDisplay + "</div>";
    ...
    panel.querySelector(".chat-x").addEventListener("click", function () { setOpen(false); });

    panel.addEventListener("click", function (e) {
      var b = e.target.closest(".chat-opt");
      if (!b) return;
      var msg = CHAT_OPTIONS[parseInt(b.getAttribute("data-i"), 10)][1];
      window.open("https://wa.me/" + CONTACT.whatsappNumber + "?text=" + encodeURIComponent(msg),
                  "_blank", "noopener");
      setOpen(false);
    });
  ...
  function buildFloaters() {
    ...
    f.innerHTML =
      '<button class="fab-wa" type="button" aria-label="Chat on WhatsApp" aria-expanded="false"><span class="fab-label">Chat on WhatsApp</span>' + wa + "</button>" +
    ...
    buildChatPanel(f.querySelector(".fab-wa"), wa);
  ...
  function registerSW() {
    ...
    // The admin panel must never be served from cache.
